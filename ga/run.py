...
def function_grid(param_grid):
    ...
    stats_array = np.zeros((max_generations, 2, 8))
                ...
                for l in range(2):
                    for m in range(2):
                        ...
                        temp_stats = evolve(
                                	...
                                	selection_func = partial(select, n = 25,choice = param_grid[1][i]),
                                	crossover_func = partial(recombine, num_splits = 3, choice = param_grid[2][i]),
                                	mutation_func = partial(mutate, p = 0.01, alleles=alleles, choice = param_grid[3][i]),
                                	replacement_func = partial(replace, n = 5, choice = param_grid[4][i]),
                        ...
                        count = count + 1
    with open('makespan_1.pickle','wb') as f:
        pickle.dump(stats_array,f,pickle.HIGHEST_PROTOCOL)
                            
    with open('makespan_1_times.pickle','wb') as f:
