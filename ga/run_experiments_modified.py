...
# need to change here for each problem:
jobs, num_machines = load_data("makespan_2.dzn")  
...
def evaluate_parameters(params):
	...
	description = ", ".join(map(partial_to_info, params))
	return stats, description

def collect_stats(population):
    fit = np.apply_along_axis(fitness_func, 1, population)
    # fit has shape (200,) --> (population_size, ) 
    return [np.max(fit), np.mean(fit)]
...
if __name__ == "__main__":
	...
	pickle.dump(results, open("results_makespan2.pickle", "wb"))
