...
def de_step(pop):
    ...
    for i, x in enumerate(pop):
        u,v,w = pop[np.random.choice(len(pop), size=3, replace=False)] # change to True if testing degenerates
        while u is x or v is x or w is x:  # comment this out if testing degenerates
            u,v,w = pop[np.random.choice(len(pop), size=3, replace=False)]  # asabove
...
param_std = np.zeros((NUM_ITERATIONS,NUM_PARAMS))
best_results = np.zeros((NUM_ITERATIONS,NUM_PARAMS))
...
#        print(best_objective)
    ...
    for j in range(0,NUM_PARAMS):
        param_std[i, j] = np.std(population[:,j])/np.abs(np.mean(population[:,j]))
#    print(param_std[i])
...
for _, x in enumerate(population):
    ...
    if(fit.objective_func(x) < best_objecive):
        best_objective = fit.objective_func(x)
        best_x = x
print(fit.objective_func(best_x))
print(best_x)
...
for i in range(0,NUM_PARAMS):
    plt.plot(range(NUM_ITERATIONS), param_std[:,i])
plt.ylim(0, 1)
plt.show()
